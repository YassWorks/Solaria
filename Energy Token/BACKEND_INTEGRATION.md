...
async function getAllProjects(req, res) {
  try {
    ...
    for (let i = 1; i < nextId; i++) {
      ...
      const documentURI = await blockchain.contract.getProjectDocumentURI(i);
  ...
  try {
    ...
    res.json({
      ...
      project: {
        ...
        kwhPerShare: Number(project.kwhPerShare),
        projectStartDate: new Date(Number(project.projectStartDate) * 1000),
    ...
    for (let i = 1; i < nextId; i++) {
      ...
      if (balance > 0) {
        ...
        portfolio.push({
          ...
          currentValue: ethers.formatEther(project.pricePerShare * balance), // Simple calculation
        });
      }
    }
...
async function getProductionHistory(req, res) {
  try {
    ...
    const events = await blockchain.contract.queryFilter(filter, 0, "latest");
  ...
  start() {
    ...
    blockchain.contract.on(
      ...
      (projectId, investor, shares, totalCost, event) => {
        ...
        this.handleSharePurchase({ projectId, investor, shares, totalCost });
      }
    );
